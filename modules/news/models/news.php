...
//		}
		...
		function getNews()
		{
			...
			$query = " SELECT id,title,content,category_id,category_alias,category_name, summary,hits,
                        alias, created_time, updated_time,seo_title,image,optimal_seo,
                        seo_keyword,seo_description,tags
    ...
    function get_new_()// không sử dụng limit để show danh sách
    {
        $query = 'SELECT * FROM ' . $this->table_name . ' WHERE published = 1  ORDER BY created_time DESC, id DESC LIMIT 5';
        global $db;
        $db->query($query);
	...
	function getNewerNewsList($cid,$created_time)
		...
		function getRelateNewsList($cid)
		{
			...
			$query = " SELECT id,title,alias, category_id,category_name,updated_time ,image,category_alias,created_time,summary
						FROM ".$fs_table -> getTable('fs_news')."
						WHERE alias <> '".$code."'
							AND category_id = $cid
							AND published = 1
	...
	/*
		...
		{
			$query = "SELECT * FROM ".$this->table_category." WHERE published = 1 AND parent_id = 0";
			global $db;
			$db->query($query);
			$result = $db->getObjectList();
			return $result;
