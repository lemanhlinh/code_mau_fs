...
//			$_SESSION[$this -> prefix.'keysearch']  =  clean($_REQUEST['keysearch']);
    ...
    function set_color($color = 1){
        ...
        $color = isset($array_panel[$color])? $array_panel[$color]:'';
        return $color;
    }
    
    function dt_col_start($col = 'col-xs-12',$begin_form = 0){
        $html = '';
        if($begin_form){
            ...
            $html .= '<form class="" role="form" action="'.$link.'" name="adminForm" method="post" enctype="multipart/form-data">';
        }
        $html .= '<div class="'.$col.'">';
        echo $html;
    }
    
    function dt_col_end($end_form = 0){
        echo '</div>';
    }
    
	function dt_form_begin($include_tablehead = 1,$panel = 1,$panel_header='',$icon='fa-cog',$color_1_17 = 1,$on_col ='',$begin_form = 0){
	    $html = '';
        ...
        if($panel_header){
            ...
            $panel_header = '<div class="box-header with-border">
                                <h3 class="box-title '.$this->set_color($color_1_17).'">
                                '.$icon.' '.$panel_header.'
                                </h3>
