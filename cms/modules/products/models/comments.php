<?php 
	...
	function __construct() {
            ...
            $this -> table_comment = FSTable_ad::_('fs_products_comments');
...
//		}
	...
	function ajax_published($published = 1) {
		...
		$rs = $this->_update ( $row, $this->table_comment, 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ' );
		echo $rs ? 1 : 0;
		if ($rs) {
	...
	function ajax_del() {
		...
		$rs = $this->_remove ( 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ', $this->table_comment );
		echo $rs ? 1 : 0;
		if ($rs) {
			$this->recal_comments ( $record_id );
		}
	...
	function recal_comments($record_id) {
		$list = $this->get_records ( 'record_id = ' . $record_id, $this->table_comment, 'id,published' );
		$total_published = 0;
	...
	function update_unread_for_comments($record_id) {
		$row ['readed'] = 1;
		$this->_update ( $row, $this->table_comment, 'record_id = ' . $record_id );
	...
	function save_comment() {
		...
		$rs = $this -> _add($row, $this->table_comment);
		echo $rs?1:0;
		if ($rs)
			$this->recal_comments ( $record_id );
		return $rs;
	...
	function edit_comment() {
		...
		$rs = $this -> _update($row, $this->table_comment,' id = '.$comment_id);
		echo $rs?1:0;
		if ($rs)
			$this->recal_comments ( $record_id );
		return $rs;
