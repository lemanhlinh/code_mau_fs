...
class SubsidiariesModelsSubsidiaries extends FSModels
{
    var $limit;
    var $page;
    ...
    function __construct()
    {
        ...
        $this->table_name = FSTable_ad::_('fs_subsidiaries');
        $this->table_field = FSTable_ad::_('fs_field');
        //synchronize
        $this->check_alias = 1;
        //$this -> table_product = 'fs_products';
        $this->table_product = FSTable_ad::_('fs_products');
        $this->arr_img_paths = array(array('resized', 152, 120, 'resize_image_fix_height'),
//                                     array('small', 180, 100, 'cut_image'),
        );
        $this->arr_img_paths_other = array(
            array('tiny', 620, 340, 'cut_image'),
            array('small', 70, 40, 'resize_image_fix_height')
        );
        $this->img_folder = 'images/subsidiaries';
    ...
    {
//			lĩnh vực
        $field = FSInput::get('field_id', 0, 'int');
        if(!$field){
            Errors::_('Bạn phải chọn lĩnh vực');
            return;
        }
        if (@$field) {
            $field_item = $this->get_record('id =' . $field, $this->table_field, 'name');
            $row['field_id'] = $field;
            $row['field_name'] = $field_item->name;
        }
